...
df['series'] = df['volume_C'] #enter the series you want to test
...
for i in range(0,len(orders)):
    for j in range(0,len(s_orders)):
        try:                
            cv_sep = datetime(2018, 1, 1)
            df_train_train = df_train.loc[df_train.date_val < cv_sep]
            df_train_test = df_train.loc[df_train.date_val >= cv_sep]
            ...
            results[i,j] = (np.abs((test_ - forecast_) / test_).mean())
